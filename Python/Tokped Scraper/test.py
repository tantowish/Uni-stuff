...
def scrape_data(param, url, condition):
    payload = [
        {
    ...
    for i in range(0, len(rows)):
        ...
        product_condition = rows[i].get('condition', '')  # Get the product condition (new or used)
        data_scrape.append(
            (product_name, price, product_rating, product_seller, product_location, product_condition)
        )
        hartot += harga
    ...
    for i in range(len(data_scrape)):
        ...
        harga = int(numeric_string)
        if (
            harga < hartot / len(data_scrape)
            and not not data_scrape[i][2].strip()
            and data_scrape[i][5].lower() == condition.lower()  # Check the product condition
        ):
